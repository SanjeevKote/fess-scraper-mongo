    ...
    if response.status_code == 200:
        ...
        with open(os.path.join(file_path, file_name + '.txt'), 'w', encoding='utf-8') as f:
            ...
            if articlePublishedDate:
                ...
                f.write(f"Publication Date not found:{articlePublishedDate}\n")
            f.write("\n" + text)
        if articlePublishedDate:
            logger.info("Publication Date: %s", articlePublishedDate)
        else:
            logger.warning("Publication Date not found : %s",articlePublishedDate)
    else:
...
# @api_view(['GET','POST'])
def Fess_economist_Post(request):
    """
